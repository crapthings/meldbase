...
```text
Identity service -> Actor in one active Workspace
Actor -> Remote collection or named RPC
Query -> snapshot, then optional realtime subscription
Change consumer -> durable background side effect
Worker -> trusted RPC handler or read-visibility policy
```

This is the product path. Storage, transport, and Worker protocol details refine
it; they do not create a competing identity, query, or authorization model.
...
3. `workspace` is Meldbase's only public isolation term. Do not introduce
   ...
   `sub` and `workspace_id` are identity-provider claim names only; public
   application and CLI language uses actor ID and workspace ID.
...
| **Database identity** | The identity that proves a database file and its recovery history belong together. | A physical backup preserves it; `Compact` intentionally creates an independent database with a new identity. It is a namespace binding, not authentication or a secret. |
| **Collection** | A named namespace of application documents. | `DB.Collection(name)` is durable database data. Collection policy is evaluated per request. |
| **Remote collection** | An authenticated application handle for one server collection. | `RemoteCollection` uses HTTP and optional realtime transport. Every operation is re-authorized and server-limited; it is not a local cache or replica. |
...
| **Index** | A declared access path for a collection query. | An index is database metadata, not an application collection or an authorization rule. Its field definition, storage publication, and recovery are durable engine concerns. |
...
| **Commit Log** | Meldbase's retained ordered history of committed database changes. | It is private engine history used for replay, durable changes, archive handoff, and replication. It is not a public collection, browser feed, WAL, or MongoDB oplog. |
...
| **Consumer checkpoint** | The last change position a named change consumer has durably acknowledged. | `Ack` advances it only after the caller's side effect is durable. It is not a browser resume token, a snapshot token, or a controller checkpoint. |
...
| **Actor** | The verified user or service making one request or Worker invocation. | Go `Actor` and TypeScript `context.actor` contain `id` and `workspaceId`/`WorkspaceID`. |
...
| **Workspace** | The active, application-defined isolation scope in which an actor is operating. It may represent a team, organization, project, or another collaborative boundary chosen by the application. | The verified workspace claim selects the actor's active workspace. Clients never choose a trusted workspace in a query or URL. |
| **Workspace ID** | The stable identifier of the actor's active workspace. | The built-in JWT configuration maps `workspace_id` to `actor.workspaceId` / `Actor.WorkspaceID`. The built-in authorizer writes the configured document field, normally `workspaceId`. |
...
| **Collection access policy** | The built-in, generic data-access rules for one collection. | It scopes the four built-in modes and optional field boundaries to the active workspace and, for owner mode, actor ID. It is not a role, membership, or workflow engine. |
...
| Term | Product and business meaning | Engineering contract |
| --- | --- | --- |
| **RPC** | A named application business operation with one explicit input. | `client.call(method, input, options?)` invokes a named RPC over HTTP by default or realtime when selected. The input is one closed `Value`; objects are the normal shape for business fields, while arrays and scalars remain valid values. A Go method map or a trusted Worker may implement it. A normal RPC is non-atomic: Meldbase does not join external effects or writes reached outside its transaction contract to its terminal result. RPC is not synonymous with Worker. |
...
| **Local SDK error** | A local misuse, lifecycle, validation, or compatibility failure before a remote business result exists. | TypeScript uses `QueryValidationError`, `MeldbaseClientClosedError`, `MeldbaseProtocolError`, and `MeldbaseWorkerProtocolError`. These do not cross the protocol and do not add a third remote error kind. |
...
| **Generation** | A physical storage metadata generation. | It can advance for maintenance such as reclamation or index storage publication without representing a new business document commit. Do not treat it as a commit token. |
