...
func MarshalPrometheus(sample Sample) []byte {
	...
	if server := sample.Server; server != nil {
		...
		writePrometheusFamily(&output, "meldbase_rpc_arguments_total", "Typed RPC arguments passed to executing handlers.", "counter", counterUint(server.RPCArguments))
		...
		writePrometheusFamily(&output, "meldbase_worker_connected", "Current authenticated server workers.", "gauge", gaugeUint(server.Worker.ConnectedWorkers))
		...
		writePrometheusFamily(&output, "meldbase_worker_registered_publications", "Current dynamically registered worker query publications.", "gauge", gaugeUint(server.Worker.RegisteredPublications))
		...
		writePrometheusFamily(&output, "meldbase_worker_policy_evaluations_total", "Worker publication policy evaluations started.", "counter", counterUint(server.Worker.PolicyEvaluations))
		writePrometheusFamily(&output, "meldbase_worker_policy_evaluations_active", "Current in-flight worker publication policy evaluations.", "gauge", gaugeUint(server.Worker.PolicyActive))
		writePrometheusFamily(&output, "meldbase_worker_policy_denied_total", "Worker publication policy evaluations denied by application logic.", "counter", counterUint(server.Worker.PolicyDenied))
		writePrometheusFamily(&output, "meldbase_worker_policy_failures_total", "Worker publication policy evaluations that failed internally.", "counter", counterUint(server.Worker.PolicyFailed))
