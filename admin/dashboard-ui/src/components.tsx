...
export function DiagnosticsTable({ events, enabled, status }: { events: DiagnosticEvent[]; enabled: boolean; status: string }) {
  ...
  return <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm"><table className="w-full min-w-[880px] border-collapse text-left text-xs"><thead className="bg-slate-50 text-[10px] uppercase tracking-[.14em] text-slate-500"><tr>{["Time", "Kind", "Plan", "Outcome", "Duration", "Work"].map((label) => <th key={label} className="px-4 py-3 font-semibold">{label}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{[...events].reverse().map((event) => <tr key={event.sequence} className="text-slate-700"><td className="px-4 py-3 text-slate-500">{time(event.capturedAt)}</td><td className="px-4 py-3 font-medium">{event.kind}</td><td className="px-4 py-3"><p>{event.stage ?? "durability"}</p><p className="mt-1 text-[10px] text-slate-400">{event.fallbackReason ?? event.planReason ?? "—"}{event.earlyStopped ? ` · early ${event.earlyStopScope ?? "stop"}` : ""}</p></td><td className={`px-4 py-3 font-semibold ${event.outcome === "success" ? "text-emerald-700" : "text-rose-700"}`}><p>{event.errorClass ?? event.outcome}</p>{(event.budgetExceeded ?? event.budgetPressure) && <p className="mt-1 text-[10px] font-medium text-amber-700">budget {event.budgetExceeded ? `exceeded: ${event.budgetExceeded}` : `pressure: ${event.budgetPressure}`}</p>}</td><td className="px-4 py-3">{duration(event.durationNanos)}</td><td className="px-4 py-3 text-right">{event.kind === "query" ? <><p>{count(event.documentsExamined)} docs / {count(event.keysExamined)} keys / {count(event.predicateSteps)} steps</p><p className="mt-1 text-[10px] text-slate-400">{count(event.duplicateCandidateIds)} duplicate / {count(event.documentsReturned)} returned</p></> : `${count(event.changes)} changes`}</td></tr>)}</tbody></table></div>;
}
...
export function queryRows(sample: AdminSample | undefined) {
  ...
  return [{ label: "Index scans", value: count(queries.indexScans) }, { label: "Collection scans", value: count(queries.collectionScans) }, { label: "ID lookups", value: count(queries.idLookups) }, { label: "Documents / returned", value: ratio(queries.documentsExamined, queries.documentsReturned) }, { label: "Keys / unique IDs", value: ratio(queries.keysExamined, queries.uniqueCandidateIds) }, { label: "Predicate steps", value: count(queries.predicateSteps) }, { label: "Duplicate / candidate IDs", value: ratio(queries.duplicateCandidateIds, queries.candidateIds) }, { label: "Early stops", value: count(queries.earlyStops) }, { label: "Budget pressure / rejected", value: `${count(queries.budgetPressureEvents)} / ${count(queries.budgetRejections)}` }, { label: "Retained sort bytes", value: bytes(queries.sortBytes) }, { label: "Planner total", value: count(queries.total) }];
}
...
export function transportRows(sample: AdminSample | undefined) {
  ...
  return [{ label: "Requests / sec", value: rate(rates.rpcRequestsPerSecond, true) }, { label: "Failures / sec", value: rate(rates.rpcFailuresPerSecond, true) }, { label: "Realtime connections", value: count(server.activeConnections) }, { label: "Maximum latency", value: duration(server.rpcMaxLatencyNanos) }, { label: "Idempotency replays", value: count(server.rpcIdempotencyReplays) }, { label: "Outcome unknown", value: count(server.rpcIdempotencyUnknown) }, { label: "Outbound limits", value: count(server.realtimeOutboundOverflows) }, { label: "Atomic commits", value: count(server.rpcAtomicCommits) }, { label: "Atomic rollbacks", value: count(server.rpcAtomicRollbacks) }, { label: "Worker connections", value: count(worker.connectedWorkers) }, { label: "Worker read policies", value: count(worker.registeredReadPolicies) }, { label: "Read policy denied", value: count(worker.policyDenied) }, { label: "Read policy invalidations", value: count(worker.policyInvalidations) }, { label: "Worker protocol failures", value: count(worker.protocolFailures) }];
}
