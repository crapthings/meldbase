...
export function DiagnosticsPage() {
  ...
  return <><PageTitle eyebrow="Opt-in bounded ring" title="Recent diagnostics" detail="Only slow, failed or explicitly sampled operations are retained. This panel intentionally excludes request payloads, collection names, actors and tenant values." /><div className="mb-4 flex items-center gap-2"><span className={`size-2 rounded-full ${enabled ? "bg-emerald-400" : "bg-slate-500"}`} /><span className="text-xs text-slate-400">{status}</span></div><DiagnosticsTable events={events} enabled={enabled} status={status} /></>;
}
