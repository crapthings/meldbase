...
func TestAnchorServeMTLSQuorumPartitionAndGracefulRejoin(t *testing.T) {
	...
	db, err := meldbase.OpenV2WithOptions(databasePath, meldbase.V2Options{RollbackProtection: meldbase.V2RollbackProtection{
	...
	db, err = meldbase.OpenV2WithOptions(databasePath, meldbase.V2Options{RollbackProtection: meldbase.V2RollbackProtection{AnchorStore: store, OperationTimeout: time.Second}})
	if err != nil {
		t.Fatal(err)
	}
	items = db.Collection("items")
