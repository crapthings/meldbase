...
func (s *resumeTokenService) issue(database [16]byte, principal Principal, collection string, query meldbase.QuerySpec, policyVersion string, position uint64) (string, error) {
	queryBytes, err := meldbase.MarshalQuerySpecJSON(query)
	if err != nil || policyVersion == "" || len(policyVersion) > 128 || len(principal.Subject) > 512 || len(principal.Tenant) > 512 {
		return "", errInvalidResumeToken
	}
	queryHash := sha256.Sum256(queryBytes)
	claims := resumeClaims{
		Version: 1, Database: hex.EncodeToString(database[:]), Subject: principal.Subject, Tenant: principal.Tenant,
...
func (s *resumeTokenService) validate(token string, database [16]byte, principal Principal, collection string, query meldbase.QuerySpec, policyVersion string) (uint64, error) {
	...
	queryHash := sha256.Sum256(queryBytes)
	if claims.Version != 1 || claims.Database != hex.EncodeToString(database[:]) || claims.Subject != principal.Subject ||
		claims.Tenant != principal.Tenant || claims.Collection != collection || claims.QueryHash != hex.EncodeToString(queryHash[:]) ||
