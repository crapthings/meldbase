...
import {
  ...
  exactKeys,
  normalizeBaseURL,
  normalizeRealtimeOrigin,
  positiveLimit,
  record,
  throwIfAborted,
  throwRemoteError,
  validRPCIdempotencyKey,
} from "./shared.js";
...
export class MeldbaseClient {
  ...
  constructor(options: ClientOptions) {
    ...
    this.#allowedRealtimeOrigins = new Set(
      (options.allowedRealtimeOrigins ?? [defaultRealtimeOrigin]).map(normalizeRealtimeOrigin),
    );
  ...
  get realtimeProtocol(): ProtocolDescriptor | undefined {
    return this.#realtime.protocol;
  }
  ...
  async call<T extends Value = Value>(method: string, input: Value, options: CallOptions = {}): Promise<T> {
    ...
    if (options.transport !== undefined && options.transport !== "http" && options.transport !== "realtime")
      throw new Error("Invalid RPC transport");
    if (options.idempotencyKey !== undefined && !validRPCIdempotencyKey(options.idempotencyKey))
      throw new Error("Invalid RPC idempotency key");
    if (options.transport === "realtime")
      return this.#realtime.call<T>(method, safeInput, options.signal, options.idempotencyKey);
    ...
    const body = JSON.stringify({
      ...
      method,
      input: encodeValue(safeInput),
    });
    let response: Response;
    try {
      response = await this.#fetch(`${this.#baseUrl}/v1/rpc`, {
        method: "POST",
        headers,
        body,
    ...
    try {
      if (!response.ok) {
        ...
        if (
          record(body) &&
          exactKeys(body, ["v", "type", "requestId", "error"]) &&
          body.v === MELDBASE_PROTOCOL_VERSION &&
          body.type === "error" &&
          body.requestId === requestId
        ) {
          const error = decodeWireError(body.error);
          throw error.kind === "business"
            ? new MeldbaseError(error.code, error.data)
            : new MeldbaseInternalError(error.code, response.status, "RPC");
        }
      ...
      if (
        !record(body) ||
        !exactKeys(body, ["v", "type", "requestId", "result"]) ||
        body.v !== MELDBASE_PROTOCOL_VERSION ||
        body.type !== "result" ||
        body.requestId !== requestId ||
  ...
  async fetchQuery<T extends Document>(collection: string, query: QuerySpec, signal?: AbortSignal): Promise<T[]> {
    ...
    if (
      !record(body) ||
      !exactKeys(body, ["version", "documents"]) ||
      body.version !== 1 ||
  ...
  async count(collection: string, query: QuerySpec, signal?: AbortSignal): Promise<CountResult> {
    ...
    if (
      !record(body) ||
      !exactKeys(body, ["version", "count", "capped"]) ||
      body.version !== 1 ||
      !boundedCount(body.count) ||
  ...
  async groupCount(
    ...
    const response = await this.#fetch(
      `${this.#baseUrl}/v1/collections/${encodeURIComponent(collection)}/group-count`,
      {
        method: "POST",
        headers: await this.headers(),
        body: JSON.stringify({ version: 1, query: encodeQuerySpec(query), groupBy }),
        ...(signal ? { signal } : {}),
      },
    );
    ...
    if (
      !record(body) ||
      !exactKeys(body, ["version", "groups", "capped"]) ||
      body.version !== 1 ||
      !Array.isArray(body.groups) ||
      body.groups.length > 100 ||
      typeof body.capped !== "boolean"
    )
      throw new Error("Malformed group count response");
    const groups = body.groups.map((item) => {
      if (!record(item) || !exactKeys(item, ["key", "count"]) || item.key === undefined || !boundedCount(item.count))
        throw new Error("Malformed group count entry");
      return { key: decodeValue(item.key), count: item.count };
    });
    return { groups, capped: body.capped };
  }

  async insertOne(collection: string, document: InputDocument, signal?: AbortSignal): Promise<Document> {
    this.assertOpen();
    assertCollectionName(collection);
    ...
    try {
      ...
      if (
        !record(result) ||
        !exactKeys(result, ["version", "document"]) ||
        result.version !== 1 ||
        result.document === undefined
      )
        throw new Error("Malformed insert response");
      const inserted = decodeDocument(result.document);
  ...
  async executeMutation(
    collection: string,
    action: "updateOne" | "updateMany" | "deleteOne" | "deleteMany",
    query: QuerySpec,
    update?: MutationSpec,
    signal?: AbortSignal,
    ...
    assertCollectionName(collection);
    throwIfAborted(signal);
    if (action !== "updateOne" && action !== "updateMany" && action !== "deleteOne" && action !== "deleteMany")
      throw new Error("Invalid mutation action");
    if ((action.startsWith("update") && update === undefined) || (action.startsWith("delete") && update !== undefined))
    ...
    const body = JSON.stringify({
      version: 1,
      action,
      query: encodeQuerySpec(query),
      ...(update ? { update: encodeMutationSpec(update) } : {}),
    });
    let response: Response;
    try {
      response = await this.#fetch(`${this.#baseUrl}/v1/collections/${encodeURIComponent(collection)}/mutations`, {
        method: "POST",
        headers,
        body,
        ...(signal ? { signal } : {}),
    ...
    try {
      ...
      if (action === "deleteOne" || action === "deleteMany") {
        if (!exactKeys(body, ["version", "deletedCount"]) || body.version !== 1 || !boundedCount(body.deletedCount))
          throw new Error("Malformed delete response");
        return { deletedCount: body.deletedCount };
      }
      if (
        !exactKeys(body, ["version", "matchedCount", "modifiedCount"]) ||
        body.version !== 1 ||
        !boundedCount(body.matchedCount) ||
        !boundedCount(body.modifiedCount) ||
        body.modifiedCount > body.matchedCount
      )
  ...
  private async createTicket(): Promise<RealtimeTicket> {
    const response = await this.#fetch(`${this.#baseUrl}/v1/realtime/tickets`, {
      method: "POST",
      headers: { ...(await this.headers()), accept: REALTIME_TICKET_ACCEPT },
    });
    if (!response.ok) await throwRemoteError(response, 64 * 1024, "realtime ticket");
    const body = await boundedJSON(response, 64 * 1024);
    if (
      !record(body) ||
      !exactKeys(body, body.protocol === undefined ? ["url", "ticket"] : ["url", "ticket", "protocol"]) ||
    ...
    let realtimeURL: URL;
    try {
      realtimeURL = new URL(body.url);
    } catch {
      throw new Error("Malformed realtime URL");
    }
    if (realtimeURL.username || realtimeURL.password) throw new Error("Realtime URL must not contain credentials");
    if (
      (realtimeURL.protocol !== "wss:" && realtimeURL.protocol !== "ws:") ||
      !this.#allowedRealtimeOrigins.has(realtimeURL.origin)
    )
      throw new Error("Realtime URL origin is not allowed");
    if (new URL(this.#baseUrl).protocol === "https:" && realtimeURL.protocol !== "wss:")
      throw new Error("Secure baseUrl requires wss realtime");
