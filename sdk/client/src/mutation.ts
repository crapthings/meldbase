...
export function compileUpdate(update: Update, overrides: Partial<QueryLimits> = {}): MutationSpec {
  ...
  for (const operator of operatorOrder) {
    ...
    if (operator === "$unset") {
      const unsetPaths = Array.isArray(raw)
        ? [...raw]
        : plainObject(raw)
          ? Object.entries(raw).map(([path, enabled]) => {
              if (enabled !== true) throw new QueryValidationError("$unset object values must be true");
              return path;
            })
          : fail("$unset expects a path array or object");
      if (unsetPaths.length === 0) throw new QueryValidationError("$unset cannot be empty");
      for (const path of unsetPaths.sort()) {
        validateMutationPath(path, paths);
        operations.push({ op: "unset", path });
      }
      continue;
    }
    if (!plainObject(raw) || Object.keys(raw).length === 0)
      throw new QueryValidationError(`${operator} expects a non-empty object`);
    for (const path of Object.keys(raw).sort()) {
      ...
      if (operator === "$inc" && typeof value !== "number" && typeof value !== "bigint")
        throw new QueryValidationError("$inc expects numeric values");
...
export function encodeMutationSpec(spec: MutationSpec): WireMutationSpec {
  return {
    ...
    operations: spec.operations.map((operation) =>
      operation.op === "unset" ? { ...operation } : { ...operation, value: encodeValue(operation.value) },
    ),
  };
}
...
export function decodeMutationSpec(input: unknown, overrides: Partial<QueryLimits> = {}): MutationSpec {
  ...
  if (
    ...
    input.operations.length > limits.maxNodes
  )
    throw new QueryValidationError("Malformed mutation envelope");
  const paths: string[] = [];
  const operations = input.operations.map((raw): MutationOperation => {
    if (!plainObject(raw) || typeof raw.op !== "string" || typeof raw.path !== "string")
      throw new QueryValidationError("Malformed mutation operation");
    validateMutationPath(raw.path, paths);
    if (raw.op === "unset") {
      onlyKeys(raw, ["op", "path"]);
      return { op: "unset", path: raw.path };
    }
    if (raw.op !== "set" && raw.op !== "inc" && raw.op !== "push" && raw.op !== "pull")
      throw new QueryValidationError(`Unknown mutation operation: ${raw.op}`);
    onlyKeys(raw, ["op", "path", "value"]);
    const value = decodeValue(raw.value);
    if (valueByteLength(value) > limits.maxStringBytes) throw new QueryValidationError("Update value is too large");
    if (raw.op === "inc" && typeof value !== "number" && typeof value !== "bigint")
      throw new QueryValidationError("inc expects numeric wire value");
...
export function applyMutation<T extends Document>(document: T, spec: MutationSpec): T {
  ...
  for (const operation of spec.operations) {
    if (operation.op === "unset") {
      unsetPath(output, operation.path);
      continue;
    }
    if (operation.op === "set") {
      setPath(output, operation.path, operation.value);
      continue;
    }
    if (operation.op === "inc") {
      ...
      setPath(
        output,
        operation.path,
        current.found ? addNumbers(current.value as Value, operation.value) : operation.value,
      );
      continue;
    }
    ...
    setPath(
      output,
      operation.path,
      current.value.filter((item) => !valueEquals(item, operation.value)),
    );
  }
...
function validateMutationPath(path: string, previous: string[]): void {
  ...
  for (const existing of previous)
    if (existing === path || existing.startsWith(`${path}.`) || path.startsWith(`${existing}.`))
      throw new QueryValidationError(`Conflicting update paths: ${existing} and ${path}`);
  previous.push(path);
}
...
function setPath(document: Record<string, Value>, path: string, raw: Value): void {
  ...
  for (const part of parts.slice(0, -1)) {
    ...
    if (value === undefined) {
      const child = Object.create(null) as Record<string, Value>;
      current[part] = child;
      current = child;
      continue;
    }
    if (
      ...
      throw new QueryValidationError("Update path traverses a non-object");
...
function addNumbers(left: Value, right: Value): number | bigint {
  if (
    ...
    throw new QueryValidationError("inc target is not numeric");
...
function exactNumber(value: bigint): number {
  ...
  if (!Number.isSafeInteger(converted) || BigInt(converted) !== value)
    throw new QueryValidationError("Mixed Int64/Float64 increment would lose precision");
  return converted;
}
...
function onlyKeys(value: Record<string, unknown>, allowed: readonly string[]): void {
  ...
  for (const key of Object.keys(value))
    if (!set.has(key)) throw new QueryValidationError(`Unexpected mutation field: ${key}`);
}
