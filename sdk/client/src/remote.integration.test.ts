...
function startDevelopmentServer(): Promise<RunningServer> {
  return startServer(async (directory) => ({ args: ["serve", "--db", `${directory}/app.meld`, "--addr", "127.0.0.1:0", "--dev-no-auth"] }));
}
...
async function startSDKWorker(server: RunningServer): Promise<RunningWorker> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.once("exit", (code, signal) => { clearTimeout(timer); reject(new Error(`Meldbase SDK worker exited before startup (code=${code} signal=${signal}): ${output}`)); });
...
test("TypeScript remote mutations and RPC interoperate with live Go and Node worker servers", { timeout: 30_000 }, async () => {
  ...
  try {
    ...
    assert.deepEqual(await todos.updateOne({ _id: inserted._id }, { $set: { done: true, title: "complete" } }), { matchedCount: 1, modifiedCount: 1 });
