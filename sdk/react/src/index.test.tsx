...
import { encodeDocument } from "@meldbase/client/internal";
...
test("useLiveQuery renders remote initial data, transitions live, and unsubscribes on unmount", async (t) => {
  ...
  function View() {
    const result = useLiveQuery(query, {
      initialData: [{ _id: "00000000000000000000000000000001", rank: 0n, title: "cached" }],
    });
    renders.push({
  ...
  try {
    ...
    await act(async () => {
      ...
      socket.message({
        ...
        subscriptionId: "todos-subscription",
        token: "token-1",
    ...
    const renderCount = renders.length;
    assert.equal(
      socket.sent.some((frame) => (JSON.parse(frame) as { readonly type?: string }).type === "unsubscribe"),
      true,
    );
